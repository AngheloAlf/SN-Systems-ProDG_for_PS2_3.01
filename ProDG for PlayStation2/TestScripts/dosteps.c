...
// ADB: dosteps.c
//
// An example of the use of STEP/STEVOVER functions in a debugger script
// Also a nice example of the use of a callback to hook TARGETATN events
//
// Note this script module has no main() function so if you run this it will just load the functions
...
// some global variables we will use to communicate between main script functions and callback code
int	_nsteps;
int	_maxsteps;
int	_slflag;
...
int SUCallback(SNPARAM message, SNPARAM param0, SNPARAM param1, SNPARAM param2)
{
	...
	if(message == SM_TARGETATN)
	{
		if(param0 == 0 && !SNIsRunning(0) )	// if it's the main core CPU and it is stopped
		{
			...
			SNSetCallback(NULL); // so cancel any callback and return the pane to immediate mode
		}
	}
	return 0;
}
